...
function formatForecastTitle({ label, date }) {
  if (!label) {
    return date;
  }
  return `${label}（${date}）`;
...
function createForecastItem({ title, max, min, code }) {
...
async function fetchWeather(latitude, longitude, label) {
  ...
  try {
    ...
    const forecastItems = daily.time.map((date, index) => ({
      ...
      label: index === 0 ? '今日' : index === 1 ? '明日' : undefined,
    ...
    forecastItems.slice(0, 2).forEach((item) => {
      forecastList.appendChild(
        createForecastItem({
          title: formatForecastTitle(item),
          max: item.max,
          min: item.min,
          code: item.code
        })
    ...
    showWeatherCard();
    showForecastCard();
    setStatus(`${locationName}の天気を表示しています。`, 'success');
  } catch (error) {
    console.error(error);
